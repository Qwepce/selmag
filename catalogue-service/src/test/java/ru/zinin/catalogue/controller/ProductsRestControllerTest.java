...
class ProductsRestControllerTest {
    ...
    void findAllProducts_ReturnsProductList() {
        ...
        //when
        var result = this.productsRestController.findAllProducts(filter);
        //then
        assertEquals(HttpStatus.OK, result.getStatusCode());
        assertEquals(List.of(new Product(1, "Первый товар", "Описание первого товара"),
                new Product(2, "Второй товар", "Описание второго товара")), result.getBody());
    }
    ...
    void createProduct_RequestIsValid_ReturnsNoContent() throws BindException {
        ...
        var urlComponentBuilder = UriComponentsBuilder.fromUriString("http://localhost");

        doReturn(new Product(1, "Новое название", "Новое описание"))
                .when(this.productService).createProduct("Новое название", "Новое описание");
        //when
        var result = this.productsRestController.createProduct(payload, bindingResult, urlComponentBuilder);
    ...
    void createProduct_RequestIsInvalid_ReturnsBadRequest() {
        ...
        var exception = assertThrows(BindException.class,
                () -> this.productsRestController.createProduct(payload, bindingResult, urlComponentBuilder));
